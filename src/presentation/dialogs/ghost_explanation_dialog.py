...
class GhostExplanationDialog(QDialog):
    ...
    def _interpret_metric(self, key, value):
        ...
        # DEEP DIVE: SONAR SECTORS
            ...
            k_low = key.lower()
            # Support for both English and Spanish keywords
            if any(x in k_low for x in ["strength", "fortaleza", "maestra"]):
                interp = MESSAGES.EXPLANATIONS.SECTOR_STRENGTH
            elif any(x in k_low for x in ["auth", "identidad"]):
                interp = MESSAGES.EXPLANATIONS.SECTOR_AUTH
            elif any(x in k_low for x in ["health", "salud"]):
                interp = MESSAGES.EXPLANATIONS.SECTOR_HEALTH
            elif any(x in k_low for x in ["rotation", "rotación"]):
                interp = MESSAGES.EXPLANATIONS.SECTOR_ROTATION
            elif any(x in k_low for x in ["intel", "inteligencia"]):
                interp = MESSAGES.EXPLANATIONS.SECTOR_INTEL
            elif any(x in k_low for x in ["sync", "nube"]):
                interp = MESSAGES.EXPLANATIONS.SECTOR_SYNC
            elif any(x in k_low for x in ["records", "registros", "digital"]):
                interp = MESSAGES.EXPLANATIONS.SECTOR_RECORDS
            elif any(x in k_low for x in ["risk", "riesgo"]):
