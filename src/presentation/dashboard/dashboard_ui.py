...
class DashboardUI:
    ...
    def _module_dashboard(self):
        ...
        if self.current_role == "admin":
            ...
            self.card_total, self.stat_total_val = mk_info_tile("TOTAL CREDENTIALS", "🔑", "#3b82f6")
            self.card_weak, self.stat_weak_val = mk_info_tile("VULNERABILITIES", "⚠️", "#ef4444")
            self.card_age, self.stat_age_val = mk_info_tile("AVG AGE (DAYS)", "📅", "#f59e0b")
        ...
        if self.current_role == "admin":
            self.btn_go_vault = mk_quick_btn("👥", "USUARIOS", "primary")
            self.btn_go_activity = mk_quick_btn("📜", "AUDITORÍA", "ai_sec")
    ...
    def _show_ai_guardian_explanation(self):
        ...
        # Parse risks from the label string "🛡️ 0 RISKS"
        risks_txt = self.lbl_dot_count.text()
        
        # Get live values from Radar
        radar_vals = self.ai_radar._values
        radar_labels = self.ai_radar._labels
        
        data = {
            MESSAGES.EXPLANATIONS.AI_STATUS: self.lbl_ai_status.text(),
