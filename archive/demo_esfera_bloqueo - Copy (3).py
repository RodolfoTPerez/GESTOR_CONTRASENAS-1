...
class HyperRealVaultCore(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # --- 2. DIBUJAR ESFERA DE NEÓN (Azul Ultra Eléctrico) ---
        # Un azul mucho más saturado y vibrante (Electric Blue)
        main_color = QColor(0, 100, 255) 
        scale_factor = 0.9 + (0.3 * pulse_val)
        
        # Glow Atmosférico (Más denso)
        glow_radius = 220 + (110 * pulse_val)
        glow_alpha = int(40 + 80 * pulse_val)
        glow = QRadialGradient(cx, cy, glow_radius)
        glow.setColorAt(0, QColor(0, 80, 255, glow_alpha))
        ...
        # Dibujar Red (Malla) de la esfera (Más gruesa y brillante)
        num_long = 24
        pen = QPen()
        for i in range(len(projected_points)):
            ...
            pen.setColor(QColor(0, 120, 255, max(30, alpha))) # Azul Eléctrico Puro
            pen.setWidthF(0.8 + 1.2 * pulse_val) 
            painter.setPen(pen)
        ...
        # Dibujar Puntos y Partículas (Más grandes y brillantes)
        painter.setPen(Qt.NoPen)
        for p in projected_points:
            if p[2] > 0: 
                p_alpha = int(255 * (p[2]/160) * pulse_val)
                painter.setBrush(QColor(0, 255, 255, max(0, p_alpha))) # Cian brillante para los nodos
                size = (1.8 + 2.8 * pulse_val) 
