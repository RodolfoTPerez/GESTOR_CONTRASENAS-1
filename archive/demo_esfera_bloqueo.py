...
# print(f"DEBUG: sys.path added: {ROOT_DIR}")
...
class HyperRealVaultCore(QWidget):
    ...
    # [CAMBIO] Default neutro mientras carga
    def __init__(self, vault_name="..."):
        ...
        self._check_connectivity() # Primera verificación inmediata
    ...
    def _fetch_real_vault_name(self):
        ...
        try:
            ...
            um = UserManager(sm)
            print(">>> [LockScreen] UserManager initialized. Querying 'vaults' table...")
            ...
            try:
                ...
                print(f">>> [LockScreen] Supabase Response: {res.data}")
                if res.data and len(res.data) > 0:
                    ...
                    if official_name:
                        ...
                        print(f">>> [LockScreen] SUCCESS: Nombre sincronizado: {self.vault_name}")
                else:
                    print(">>> [LockScreen] WARNING: Supabase returned empty data for 'vaults'")
    ...
    def _check_connectivity(self):
        ...
        try:
            # Intentamos conectar con los servidores de Google para verificar internet real
    ...
    def _get_color(self, r, g, b, a=255):
        ...
        if self._is_online:
            ...
            # Shift hacia rojo táctico manteniendo luminosidad/alpha aproximada
            # Usamos un rojo intenso (225, 29, 72) como base para offline
    ...
    def _draw_status_shield(self, painter, cx, cy, scale):
        """Dibuja un escudo de estado robusto en la esquina superior derecha."""
        ...
        pulse_val = abs(math.sin(self._pulse_time * 2))
        
        # Selección de color según conectividad
        if self._is_online:
            base_col = QColor(57, 255, 20, 100) # Verde Neón Traslúcido
            border_col = QColor(57, 255, 20, 200)
            glow_col = QColor(57, 255, 20, 40)
        else:
            # Rojo Ghost con parpadeo de inestabilidad
        ...
        # 2. Crear Path de Escudo Robusto (Multicapa)
        ...
        # Capa Exterior (Marco Grueso)
        outer_path = get_shield_path(sw, sh)
        painter.setPen(QPen(border_col, 3 * scale))
        painter.setBrush(QColor(0, 0, 0, 100)) # Fondo oscuro para solidez
        ...
        # --- AÑADIR TUERCAS INDUSTRIALES (3 Puntos) ---
        def draw_nut(nx, ny, nsize):
            ...
            # Dibujar Tuerca con efecto metálico/sólido
            painter.setPen(QPen(border_col, 1.5 * scale))
            painter.setBrush(QColor(20, 20, 25, 230)) # Metal oscuro
            painter.drawPath(nut_path)
            # Brillo central de la tuerca
        ...
        # 2. Superior Derecha
        draw_nut(tx + nut_dist_x, ty - nut_dist_y, nut_size)
        # 3. Inferior Central (en la punta)
        draw_nut(tx, ty + sh * 0.4, nut_size)

        # Símbolo de Verificación Central (Check o X)
    ...
    def paintEvent(self, event):
        ...
        # --- TÍTULO SUPERIOR (Nombre de la Bóveda / Empresa) ---
        ...
        # --- 1.2 ESCUDO DE ESTADO (TOP RIGHT) ---
        ...
        # 2. Branding App (Abajo a la Izquierda) - MÁS GRANDE & DINÁMICO
        app_font = QFont("Segoe UI", int(16 * scale), QFont.Bold) # Aumentado a 16
        app_font.setLetterSpacing(QFont.AbsoluteSpacing, 3 * scale)
        painter.setFont(app_font)
        painter.setPen(self._get_color(0, 255, 255, 140)) # Más visible
        margin = 35 * scale
        # Usamos self.vault_name que viene de Supabase (o Cache)
