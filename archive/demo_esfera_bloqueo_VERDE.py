...
class NeonSphereLock(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # 4. Dibujar RED con brillo variable MARCADO (Vuelve la intensidad)
        num_long = 24
        pen = QPen()
        for i in range(len(projected_points)):
            ...
            pen.setWidthF(0.5 + 1.0 * pulse) # El grosor vuelve a latir
            painter.setPen(pen)
        ...
        # 5. Puntos brillantes que crecen claramente
        painter.setPen(Qt.NoPen)
        for p in projected_points:
            if p[2] > 0: 
                p_alpha = int(255 * (p[2]/160) * pulse)
                painter.setBrush(QColor(16, 185, 129, max(0, p_alpha)))
                size = (1.5 + 2.5 * pulse) 
