...
class HyperRealVaultCore(QWidget):
    ...
    def _draw_hyper_floor(self, painter, cx, cy, pulse):
        """Dibuja el suelo de bóveda con estética Glassmorphism Azul Traslúcido"""
        
        # 1. Resplandor base de profundidad (Mucho más sutil y etéreo)
        base_glow = QRadialGradient(cx, cy + 220, 600)
        base_glow.setColorAt(0, QColor(0, 80, 255, 35))
        base_glow.setColorAt(1, Qt.transparent)
        painter.setBrush(base_glow)
        painter.setPen(Qt.NoPen)
        painter.drawEllipse(int(cx - 600), int(cy - 100), 1200, 600)

        # 2. Capas de la bóveda (Transparencias reducidas para no opacar)
        # [radio, grosor, color(rgba), velocidad, estilo, ticks]
        layers = [
            [520, 1.0, QColor(0, 150, 255, 40), 0.2, Qt.SolidLine, True],
            [480, 6, QColor(0, 80, 255, 50), -0.4, Qt.SolidLine, False],
            [420, 1.5, QColor(59, 130, 246, 70), 0.6, Qt.DashLine, True],
            [350, 3, QColor(0, 255, 255, 80), 0.3, Qt.SolidLine, False],
            [280, 1.0, QColor(0, 100, 255, 50), -0.1, Qt.DotLine, False],
            [200, 8, QColor(0, 255, 255, 100), 0.8, Qt.CustomDashLine, False],
        ]
        ...
        for r, w, col, speed, style, ticks in layers:
            ...
            # Sombra/Brillo inferior muy leve
            painter.setPen(QPen(QColor(0, 20, 80, 40), w))
            painter.drawEllipse(rect)
            
            # Anillo principal traslúcido
            pen = QPen(col, w)
            ...
            # Engranajes suavizados
            if w > 2:
                painter.setPen(QPen(QColor(col.red(), col.green(), col.blue(), 120), w))
                for t in range(12):
                    angle = (t * (360/12) + rot)
            ...
            if ticks:
                ...
                for t in range(72):
                    ...
                    if (angle % 360) < 180:
                        painter.drawArc(rect, int(angle * 16), 12)
            
            # Escaneo Radial Muy Sutil (Blanco Fantasma)
            scan_grad = QConicalGradient(cx, cy + 220, rot * 2)
            scan_grad.setColorAt(0, Qt.transparent)
            scan_grad.setColorAt(0.1, QColor(255, 255, 255, int(100 * pulse)))
